...
def load_corpus(path):
    """Loads our paraghaphs into a dict of key: pid, value: text-paragraph."""
    full_df = pd.read_pickle('./pkl_files/full_df.pkl')
    df = full_df.loc[:, ['docno', 'start_idx', 'length', 'txt']]
    df['pid'] = df.loc[:, ['docno', 'start_idx', 'length']].astype(str).apply(lambda x: '_'.join(x), axis=1)
    corpus = df.loc[:, ['pid', 'txt']].drop_duplicates('pid', 'first').set_index('pid').to_dict()['txt']
    return corpus
